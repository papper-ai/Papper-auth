...
async def registration(registration_request: auth_models.RegistrationRequest,
    ...
    if user:
        ...
        await user_repository.add(
            repo_models.User(**registration_request.model_dump(exclude={"secret"}), user_id=user_id,
...
async def login_for_access_token(logging_data: auth_models.LoggingData,
                                 user_repository: UserRepository = Depends(UserRepository)):
    user = await utils.authenticate_user(logging_data.username, logging_data.password, user_repository)
    if not user:
        raise HTTPException(
